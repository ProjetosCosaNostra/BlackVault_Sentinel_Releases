...
if (-not (Test-Path -LiteralPath $Base)) {
    throw "BlackGold Project Runner nao esta instalado em $Base"
}
...
if (-not (Test-Path -LiteralPath $TempPath)) {
    throw 'Falha ao baixar Runner V1.0.1'
}

$text = Get-Content -LiteralPath $TempPath -Raw
if ($text -notmatch "RunnerVersion\s*=\s*'1\.0\.1'") {
    throw 'Arquivo baixado nao e Runner V1.0.1'
}
...
Write-Host ''
Write-Host 'BLACKGOLD PROJECT RUNNER ATUALIZADO PARA V1.0.1' -ForegroundColor Green
Write-Host 'A tarefa foi reativada e a nova rodada foi disparada.' -ForegroundColor Cyan
Write-Host 'Job aguardando: orcamento-smoke-v1-002' -ForegroundColor Yellow
