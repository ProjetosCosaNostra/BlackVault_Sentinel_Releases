...
function Get-AndroidRoot([object]$Project) {
    ...
    $configured = Join-Path $Project.root $Project.android_project

    if (Test-Path -LiteralPath $configured) {
        $configuredWrapper = Join-Path $configured 'gradlew.bat'
        if (Test-Path -LiteralPath $configuredWrapper) {
            return $configured
        }
    }
    ...
    foreach ($wrapper in @(
        Get-ChildItem -LiteralPath $Project.root -Filter 'gradlew.bat' -File -Recurse -ErrorAction SilentlyContinue
    )) {
        $candidateRoot = $wrapper.Directory.FullName
        $hasApp = (
            (Test-Path -LiteralPath (Join-Path $candidateRoot 'app\build.gradle')) -or
            (Test-Path -LiteralPath (Join-Path $candidateRoot 'app\build.gradle.kts'))
        )
        $hasSettings = (
            (Test-Path -LiteralPath (Join-Path $candidateRoot 'settings.gradle')) -or
            (Test-Path -LiteralPath (Join-Path $candidateRoot 'settings.gradle.kts'))
        )

        if ($hasApp -and $hasSettings) {
            $candidates += [pscustomobject]@{
                Root = $candidateRoot
                Length = $candidateRoot.Length
                AndroidHint = $(if ($candidateRoot -match '(?i)[\\/]android
...
function Invoke-Job([object]$Job,[string]$JobLog) {
    ...
    switch ($action) {
        ...
        'project_status' {
            ...
            $resolvedAndroidRoot = $null
            try {
                if ($project.android_project) {
                    $resolvedAndroidRoot = Get-AndroidRoot $project
                }
            }
            catch {}
            ...
            return [ordered]@{
                ...
                resolved_android_root=$resolvedAndroidRoot
...
try {
    ...
    if ($candidates.Count -gt 0) {
        $selected = $candidates |
            Sort-Object AndroidHint,Length |
    ...
    throw "ANDROID_PROJECT_NOT_FOUND: $configured"
}
