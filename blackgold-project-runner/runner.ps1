...
function Get-RunningTargetSerial([object]$Project) {
    ...
    foreach ($row in $rows) {
        ...
        if (-not $text -or $text.StartsWith('List of devices')) { continue }

        $parts = @($text -split '\s+')
        if ($parts.Count -lt 2) { continue }
        if ([string]$parts[1] -ne 'device') { continue }

        $serial = [string]$parts[0]

        if ([string]$Project.id -eq 'orcamento_no_ponto' -and $serial -eq 'emulator-5554') {
            continue
        }
        ...
        if ($cleanNames.Count -eq 0) { continue }

        $avdName = [string]$cleanNames[0]
        if ($avdName -eq [string]$Project.avd) {
    ...
    if ($found.Count -gt 1) {
        $pairs = ($found | ForEach-Object { ([string]$_.serial) + '=' + ([string]$_.avd) }) -join ','
        throw "AVD_SERIAL_AMBIGUOUS: $pairs"
    }

    if ($found.Count -eq 1) {
        return [string]$found[0].serial
    }
...
function Invoke-VisualGate([object]$Project,[object]$Job,[string]$JobLog) {
    ...
    $ensureLaunch = $false
    if ($Job.args -and $Job.args.PSObject.Properties['ensure_launch']) {
        $ensureLaunch = [bool]$Job.args.ensure_launch
    }
    ...
    if ($ensureLaunch) {
        ...
        if ($Job.args.PSObject.Properties['activity'] -and $Job.args.activity) {
            $activity = [string]$Job.args.activity
        }
