...
try {
    ...
    if ($LockStream) { $LockStream.Dispose() }
        ...
        if ($Project.id -eq 'orcamento_no_ponto' -and $serial -eq 'emulator-5554') {
            continue
        }
    ...
    if ($matches.Count -gt 1) {
        $pairs = ($matches | ForEach-Object { $_.serial + '=' + $_.avd }) -join ','
